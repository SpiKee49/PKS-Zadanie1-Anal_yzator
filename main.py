...
def analyzeArp(packets):
    complete_comms = []
    incomplete_comms = []
    arp_packets = list(filter(lambda packet: packet['ether_type'] == 'ARP', packets))
...
def analyzeTftp(packets):
    ...
    udp_packets = list(filter(lambda packet: packet['ether_type'] == 'IPv4' and packet['protocol'] == 'UDP', packets))
...
def getHexdump(hex):
    ...
    for i in range(0,math.ceil(len(hex)/16)):
        ...
        hexCopy[0+(i*16)] = '\n'+hexCopy[0+(i*16)] if i != 0 else hexCopy[0+(i*16)]
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--destination", help="Path to source .pcap file", default="./vzorky/eth-1.pcap")
    parser.add_argument("-p", "--protocol", help="Select specific protocol to analyze")
       ...
       # analyzing ethernet II frames
        if 'ETHERNET II' in pkt['frame_type']:
            ...
            if 'IPv4' in pkt['ether_type']:
                ...
                pkt['src_port'] = int(hexDecoded[14+ihl*4]+hexDecoded[14+ihl*4+1],16)
                pkt['dst_port'] = int(hexDecoded[14+ihl*4+2]+hexDecoded[14+ihl*4+3],16)
                ...
                if type(getAppProtocol(pkt['src_port'],pkt['protocol'])) == str or  type(getAppProtocol(pkt['dst_port'],pkt['protocol']))== str:
                    pkt['app_protocol'] = getAppProtocol(pkt['src_port'],pkt['protocol']) if type(getAppProtocol(pkt['src_port'],pkt['protocol'])) == str else getAppProtocol(pkt['dst_port'],pkt['protocol']) 
                ...
                pkt['arp_opcode'] = getArpOp(int(hexDecoded[20]+hexDecoded[21],16))
        ...
        pkt['hexa_frame']= ruamel.yaml.scalarstring.LiteralScalarString(getHexdump(hexDecoded))
    ...
    if args.protocol:
        if (args.protocol == 'TFTP'):
            ...
            data.update(analyzeArp(packets=list(filter(lambda packet: packet['frame_type'] == 'ETHERNET II', packets))))
        elif (args.protocol == 'RIP'):
            data.update(filterRip(packets=list(filter(lambda packet: packet['frame_type'] == 'ETHERNET II', packets))))
        else:
